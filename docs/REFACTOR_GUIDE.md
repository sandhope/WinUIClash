...
2. UI 修改 Windows 注册表开启系统代理, 若 TUN 设置开启，则创建虚拟网卡
│
▼
┌───────────────────────────────────────────────┐
...
4. UI 关闭 Windows 系统代理，若 TUN 设置开启，则卸载虚拟网卡
...
- **开始按钮**：按用户设置的出站模式激活/断开代理——PATCH 核心 `mode`（激活 = 用户选定的 `rule`/`global`，断开 = `direct`）；并按 `AppSettings.TunMode` 创建/卸载虚拟网卡（**不翻转用户的 TUN 设置**，TUN 开关只是偏好，详见 3.4）。**绝对不 toggle 进程**，也**绝不回写出站模式用户设置**（出站模式是独立的单一来源，详见 3.3）。
...
- **连接态即时生效**：系统代理开关不只是偏好记录——若代理已处于连接状态（`IsRunning == true`），此时切换系统代理开关会**立即**开启/关闭 Windows 系统代理，无需断开重连；未连接时仅持久化偏好，等下次「开始」时由连接流程统一施加。

### 3.3 出站模式（OutboundMode）：单一可观察来源

出站模式（Rule / Global / Direct）是一个**独立且持久化的“用户设置状态”**，与“开始/停止按钮”（`IsRunning`）是两件事，切勿混淆：

- **唯一真相来源**：`AppSettings.OutboundMode`（字符串 `"rule"` / `"global"` / `"direct"`，随设置持久化、自动保存）。它本身就代表“用户设置的状态”。
- **谁能改、在哪改**：**只有用户**可以修改，且**只能从仪表盘的模式单选框或系统托盘的出站模式子菜单**修改。核心/后台逻辑一律不得写它。
- **三处自动同步**：仪表盘三个 RadioButton（`x:Bind Vm.Settings.OutboundMode` + `StringEqualsConverter` 双向绑定）、状态栏文本、托盘子菜单勾选，都**直接读取并订阅** `AppSettings.OutboundMode` 的 `PropertyChanged`。任一处改它，三处自动刷新，无需任何手动状态互传。
- **PATCH 核心只是副作用，绝不回写 settings**：把模式 PATCH 到 mihomo（`ClashOrchestrator.SetOutboundModeAsync` → `HttpClashService`）仅作为副作用执行，**绝不反向写入 `AppSettings.OutboundMode`**。这从根上切断了“核心 → settings → UI”的回写循环，杜绝了“PATCH 失败但设置已变”“多处互相覆盖”导致的状态漂移。
- **副作用施加点唯一**：只有 `DashboardViewModel` 订阅 `AppSettings.OutboundMode` 变化并施加副作用（PATCH 核心 / 断开代理）；状态栏与托盘仅刷新各自显示，不重复 PATCH。
- **Direct = 断开代理**：用户在仪表盘/托盘把出站模式设为 `Direct` 时，若开始按钮正处于运行状态（`IsRunning`），立即触发**断开代理**（PATCH 核心为 `direct` + 关闭系统代理）。
- **开始/停止按钮不改用户设置**：开始按钮按 `AppSettings.OutboundMode` 激活代理（若用户设置恰为 `direct`，点“开始”即隐式选择 `rule` 建立连接，并把该用户主动选择回写为 `rule`）；停止按钮把核心 PATCH 为 `direct`，但**不改写**用户的出站模式偏好。
...
- **连接态即时生效**：TUN 开关不只是偏好记录——若代理已处于连接状态（`IsRunning == true`），此时切换 TUN 开关会**立即** PATCH `/configs` 创建/卸载虚拟网卡，无需断开重连；未连接时仅持久化偏好，等下次「开始」时由连接流程统一施加。
...
- [ ] **T4**: 重构 `DashboardViewModel.ToggleCoreAsync`（开始/停止按钮的核心逻辑）：
  - 当 `isStart == true`（用户点击开始）：
    1. 调用 `HttpClashService` 发送 `PATCH /configs`，将 `mode` 修改为**用户设定的出站模式**（`AppSettings.OutboundMode`，即 `rule`/`global`；若用户当前设为 `direct`，则隐式按 `rule` 连接并把该主动选择回写为 `rule`）。**除此隐式选择外不回写 `AppSettings.OutboundMode`**。
    2. 调用 `SystemProxyService.Enable()` 开启 Windows 系统代理；如果 `AppSettings.TunMode` 是 `true`， 则创建TUN 虚拟网卡。
    3. 将 UI 状态更新为 Running（显示 Pause 图标）。
  - 当 `isStart == false`（用户点击断开）：
    1. 调用 `HttpClashService` 发送 `PATCH /configs`，将 `mode` 修改为 `"direct"`（**纯副作用，不改写用户的出站模式偏好**）。
    2. 调用 `SystemProxyService.Disable()` 关闭 Windows 系统代理；如果 `AppSettings.TunMode` 是 `true`， 则卸载TUN 虚拟网卡。
    3. 将 UI 状态更新为 Stopped（显示 Play 图标）。
  - **出站模式选择器**（Rule/Global/Direct）与开始/停止按钮解耦：其唯一来源是 `AppSettings.OutboundMode`，仪表盘/状态栏/托盘三处订阅同步；用户选 `Direct` 且正在运行时触发断开代理。详见 3.3。
